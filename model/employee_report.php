...
<body>
    ...
    <div class="content">
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['from_date']) && isset($_POST['to_date'])) {
            ...
            $result_employees = mysqli_query($link, $query_employees);
            ...
            while ($employee = mysqli_fetch_assoc($result_employees)) {
                ...
                echo "<tr>
                        ...
                        <td>$total_days</td>
                        <td>$total_absents</td>
                        <td><input type='text' id='tasks_assigned_$employee_id'></td>
                        <td><input type='text' id='tasks_done_$employee_id'></td>
                        <td>
                            <button onclick='generateReport($employee_id, \"$from_date\", \"$to_date\")' class='buttona'>
                                Generate Report
                            </button>
                        </td>
                    </tr>";
        ...
        <script>
            function generateReport(employeeId, fromDate, toDate) {
                let tasksAssigned = document.getElementById('tasks_assigned_' + employeeId).value;
                let tasksDone = document.getElementById('tasks_done_' + employeeId).value;
                let url = 'generate_employee_report.php?employee_id=' + employeeId + '&from_date=' + fromDate + '&to_date=' + toDate + '&tasks_assigned=' + tasksAssigned + '&tasks_done=' + tasksDone;
